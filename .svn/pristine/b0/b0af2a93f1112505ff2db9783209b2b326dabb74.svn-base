...
import com.cal.util.SecurityHelper;
import com.cal.util.WebCookie;
...
//       }     
       ...
       public String queryAll(HttpServletRequest request, HttpServletResponse response){
    	   try {
			   ...
			   Map<Object, Object> map = calculateUnitPrice("08AL", "Plates", a.getExchangeRate());
			   ...
			   if(StringUtils.isNotBlank(request.getParameter("type"))){
	    		   Cookie userCookie = new Cookie("quote",null);           
	    		   userCookie.setPath("/");
	    		   userCookie.setMaxAge(0);
	    		   response.addCookie(userCookie);
	    		   return "calculator.jsp";
			   }		   
			   String quoteId = WebCookie.getQuote(request);
			   if(StringUtils.isNotBlank(quoteId)){
				   response.sendRedirect("http://calculator.metal-work.com/steelPlate/calculator/queryBom.do?quoteId="+quoteId+"");	 
				   return "";
			   }else{
				   return "calculator.jsp";
			   }
...
//			   AmountUnit a = amountUnitService.queryByCurrencyShorthand("USD");
			   ...
			   AmountUnit a = amountUnitService.queryById(clientOrder.getAmountUnit());
			   Map<Object, Object> map = calculateUnitPrice("08AL", "Plates", a.getExchangeRate());
			   AmountUnit amountUnit = amountUnitService.queryByCurrencyShorthand("CNY");  
			   List<GoodsTypeNormal> goodsTypeNormals = goodsTypeNormalService.queryByGoodsTypeInfoId(GOODS_TYPE_ID);
       ...
       public JsonResult<Object> increaseProduct(HttpServletRequest request, HttpServletResponse response){
    	  ...
    	  try {	
			  ...
			  List<OrderDetails> list = orderDetailsService.queryByQuoteId(quoteId);
			   ...
			   String selectUnit = request.getParameter("selectUnit");
			   ...
			   orderDetails =  CalculatingProduct.calculate(parameterMm, parameterInch,priceUnit, goodsTypeId, quantity, density);
			   orderDetails.setSelectUnit(selectUnit);
			   ...
			   Cookie cookie = new Cookie("unit",selectUnit);           
    		   cookie.setPath("/");
...
//			   String materialType = request.getParameter("materialType");
       ...
       public JsonResult<Object> saveClientOrder(HttpServletRequest request, HttpServletResponse response){
		...
		try {
  		  ...
  		  logger.info("报价询盘数据开始"+ statrTime);
			  ...
			  String currencyShorthand = request.getParameter("currencyShorthand");
			   ...
			   String partName = "Part1";
			   if(!(request.getParameter("partName").trim() == null || "".equals(request.getParameter("partName").trim()))){
				   partName = request.getParameter("partName");
			   }
			  ...
			  //存放登录数据到Cookie
    		  String str = clientOrder.getQuoteId(); 		 
    		  Cookie userCookie = new Cookie("quote",SecurityHelper.encrypt("quoteId", str));           
    		  userCookie.setPath("/");
    		  ...
    		  response.addCookie(cookie);
			   ...
			   logger.info("报价询盘数据结束花费时间"+ (endTime-statrTime));
...
//			   Integer priceUnit = Integer.parseInt(calculateUnitPrice(materialType, goodsType, a.getExchangeRate()).get("price").toString());
//			   
//			   OrderDetails orderDetails =  CalculatingProduct.calculate(parameterMm,parameterInch,priceUnit, goodsTypeId, set, density);
...
//       }
		...
		public String toUpdateOrderDetails(HttpServletRequest request, HttpServletResponse response){
			try {
				...
				ClientOrder clientOrder = clientOrderService.queryByQuoteId(orderDetails.getQuoteId());
				selectExchangeRate = amountUnitService.queryById(clientOrder.getAmountUnit()).getExchangeRate();
			    ...
			    Integer price = Integer.parseInt(calculateUnitPrice(materialInfo.getMaterialType(), materialInfo.getGoodsType(), selectExchangeRate).get("price").toString());
			    AmountUnit amountUnit = amountUnitService.queryByCurrencyShorthand("CNY");
			    List<GoodsTypeNormal> goodsTypeNormals = goodsTypeNormalService.queryByGoodsTypeInfoId(GOODS_TYPE_ID);   
				...
				List<ProcessInfo> processInfos = processInfoService.queryAll();
				  ...
				  //客户选择的单位
				  Cookie cookie = new Cookie("unit",orderDetails.getSelectUnit());           
	    		  cookie.setPath("/");
	    		  cookie.setMaxAge(60*60*24);
	    		  response.addCookie(cookie);
		...
		public JsonResult<String> updateOrderDetails(HttpServletRequest request, HttpServletResponse response){
		  	try {
				...
				String goodsType = request.getParameter("goodsType");
				   ...
				   OrderDetails o = CalculatingProduct.calculate(parameterMm,parameterInch, priceUnit, goodsTypeId, quantity, density);
				   orderDetails.setGoodsTypeId(goodsTypeId);
				   orderDetails.setMaterialId(materialInfoId);
				   orderDetails.setParameterMm(parameterMm);
				   orderDetails.setParameterInch(parameterInch);
				   ...
				   orderDetails.setProductSizeInch(o.getProductSizeInch());
				   ...
				   //更新汇率选择，一个订单只支持一个汇率
				   if(StringUtils.isNotEmpty(request.getParameter("currencyShorthand"))){
					  AmountUnit amountUnit = amountUnitService.queryByCurrencyShorthand(request.getParameter("currencyShorthand"));
					  ClientOrder clientOrder = clientOrderService.queryByQuoteId(orderDetails.getQuoteId());
					  clientOrder.setAmountUnit(amountUnit.getId());
