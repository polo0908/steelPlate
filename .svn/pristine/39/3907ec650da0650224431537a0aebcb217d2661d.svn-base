...
public class PinXiangController {

	public static Logger logger = Logger.getLogger(PinXiangController.class);
	...
	@RequestMapping("/getAllPinXiang")
	public void getAllPinXiang(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			// 翻译起始港口的中文数据
			List<PinXiang> resaultLst = translateQidianportChNameToEnName(nwList);
	...
	@RequestMapping("/getPxHotPortList")
	public void getPxHotPortList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
	...
	private List<PinXiang> translateQidianportChNameToEnName(List<PinXiang> nwList) {
		List<PinXiang> resaultLst = new ArrayList<PinXiang>();
		for (PinXiang pXiang : nwList) {
			String qidianPort = pXiang.getQidianport();// 起始港口
			String zhongdianport = pXiang.getZhongdianport();// 终点港口
			...
			if (qidianPort == null || "".equals(qidianPort.trim())) {
				pXiang.setQidianport(pXiang.getQiDianRegion());
			} else {
				pXiang.setQidianport(qidianPort);
			}
			// 判断终点港口数据是否含有中文，含有的，放入地区数据
			if (StringHelp.isHasChinese(zhongdianport)) {
				pXiang.setZhongdianport(pXiang.getZhongDianRegion());
			}
			resaultLst.add(pXiang);
		}
		return resaultLst;
	}
